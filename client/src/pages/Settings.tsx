...
export default function Settings() {
  ...
  return <AccountPageFrame kicker="IDK / SETTINGS" title={<>SET THE<br /><em>WORKBENCH.</em></>} description="Review the application preferences that shape your IDK workspace. Controls with account impact are linked to real flows; local appearance state stays on this device.">
    <div className="settings-grid">
      <section className="settings-card"><div className="settings-card-head"><MonitorCog size={19} /><span className="mono">APPEARANCE</span></div><h2>VISUAL SIGNAL</h2><p>Current theme preference: <strong>{preference.toUpperCase()}</strong>. High contrast: <strong>{highContrast ? "ON" : "OFF"}</strong>.</p><Link className="button button-outline" href="/personalization">OPEN PERSONALIZATION <ArrowUpRight size={15} /></Link></section>
      <section className="settings-card"><div className="settings-card-head"><SlidersHorizontal size={19} /><span className="mono">ANALYSIS / AI</span></div><h2>KERNEL BEHAVIOR</h2><p>Explanation depth, response style, technical vocabulary, visual guidance, and improvement suggestions are managed in Personalization.</p><Link className="button button-outline" href="/personalization">TUNE IDK <ArrowUpRight size={15} /></Link></section>
      {[
